...
当前产品面已经很完整：以 typed Python CLI 作为主接口，支持 SQLite 和 PostgreSQL 后端、Alembic 迁移、本地 FastAPI 服务，并提供一方 React Web UI（位于独立的 `lifeos-web` 仓库），让人类可以通过浏览器操作同一份已配置的 LifeOS 数据库。CLI 同时面向人类和 agent 设计，具备稳定命令语法、help-first 文档、identifier-driven 流程和可预测的文本输出。
...
本地 Web UI 是面向人类的浏览器界面，用来操作同一份 LifeOS 数据。它是一方、本地化的产品面：UI 位于独立的 `lifeos-web` 仓库，通过生成的 OpenAPI 契约与 Web API 通信。
...
如果需要浏览器访问或 HTTP 访问，可以安装可选的本地 Web API 运行时依赖。它会使用同一份 LifeOS 数据库配置：

```bash
uv tool install --upgrade "lifeos-cli[web]"
```

升级已有安装时，运行 `uv tool upgrade lifeos-cli`。如果你当初用精确版本安装（例如 `lifeos-cli==1.3.5` 或 `lifeos-cli@1.3.5`），`uv tool upgrade` 会保留该固定版本；请用 `uv tool install "lifeos-cli@latest"` 重新安装以升级到最新版本，如需 extras 则在 requirement 中保留（例如 `uv tool install "lifeos-cli[web]@latest"`）。
...
SQLite Web 写请求在读取前取得写锁，只读请求保持并发。有限锁等待耗尽后，API 回滚并返回带 `Retry-After` 的 `503`，而不是直接暴露锁错误 `500`；这不为任意请求提供自动重放或恰好一次执行保证。

Web 数据库事务与锁冲突响应统一由请求边界负责，包括 area 排序和偏好设置引起的重算；路由不独立重试或提交。为兼容现有行为，财务资产列表 GET 会初始化缺失的内置资产，因此显式声明写意图；财务格式化读取不再初始化资产。经验倍率设置在修改配置前取得 SQLite 写锁，但配置文件与数据库不是一个原子资源：后续数据库失败仍可能导致派生经验值过期，可重试设置或使用派生数据修复命令。CLI 工作流保留显式事务作用域，不适用 HTTP 错误策略。
...
Web API 没有认证，默认绑定 `127.0.0.1`；个人使用请保持 loopback 默认值。安全边界、SQLite 文件权限与数据库凭据指引见 [SECURITY.md](SECURITY.md)。
