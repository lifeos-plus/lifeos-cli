...
`data export all` writes a schema-v4 bundle atomically with owner-only (`0600`) permissions. It contains both portable active-resource JSONL and a lossless snapshot of every authoritative table, including finance data, timelog templates, relationships, and soft-deleted history. PostgreSQL exports use a `REPEATABLE READ` snapshot so the multiple table projections describe one database state, and entries are written incrementally to bound archive-construction memory. The manifest records a SHA-256 digest and row count for every required entry. Restore validates the complete archive and all row shapes, domain invariants, foreign keys, and weak endpoints before `--replace-existing` can clear any table. Legacy schema-v3 bundles remain available for merge import but cannot replace a database because those archives were partial.
